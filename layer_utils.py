...
def anchor_sizes(n_layers=4):
    ...
    """
    # s = np.linspace(0.1, 0.5, n_layers + 1) #0.1-0.5 network
    # s = np.linspace(1/16, 0.25, n_layers + 1) #1/16 - 1/4 network
    # s = np.linspace(3/16, 0.25, n_layers + 1) #3/16 - 1/4 network
    # s = np.linspace(1/16, 2/16, n_layers + 1) #1/16 - 2/16 network
    use_full_image = False
    s = np.array([4/128, 8/128,16/128,32/128])
    if use_full_image is True:
        s = np.array([4/8192, 8/8192,16/8192,32/8192])
    # s = np.linspace(3/64, 5/64, n_layers + 1) #3/64 - 5/64 network
    sizes = []
...
def anchor_boxes(feature_shape,
                 ...
                 index=0,
                 n_layers=4,
                 aspect_ratios=(1, 2, 0.5)):#(1, 2, 3, 0.5, 0.3333)):
