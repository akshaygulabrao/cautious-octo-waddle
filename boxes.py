...
def nms(args, classes, offsets, anchors):
    ...
    if nms is False:
        indexes = np.nonzero(objects)
        scores = classes[:,1]
        return objects, indexes[0], scores
    else:
...
def show_boxes(args,
    ...
    for idx in indexes:
        ...
        x = anchor[0]
        y = anchor[2]
        cv2.rectangle(img,(int(anchor[0]),int(anchor[2])), (int(anchor[0] + w), int(anchor[2]+h)), (255,0,0))
